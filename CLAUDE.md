    ...
    **`website/frontend/.env`** (used by Vite frontend):
    ...
    - **`App.jsx`** — React Router v7 routes: `/`, `/calculator`, `/contact`, `/uploadbtn`, `/join`, `/auth`, `/demo`.
    - **`assets/components/`** — Page-level JSX components (landing, calculator, uploadbtn, Join, auth, contact, animations).
    ...
    | POST | `/upload` | Multipart file upload (multer) |
    ...
    - Reed-Solomon error correction is a declared dependency but integration is in-progress.
