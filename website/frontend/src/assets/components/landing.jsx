...
const ProjectAtlasLanding = () => {
  ...
  useEffect(() => {
    const sections = ["features", "algorithm"];
  ...
  const features = [
    {
      ...
      description:
        "Advanced neural network-powered compression with lossless recovery",
    },
    {
      title: "Smart Sharding",
      description:
        "Intelligent data distribution across multiple nodes for maximum reliability",
    },
    {
      title: "Token Rewards",
  ...
  return (
    <div className="min-h-screen bg-black text-white relative overflow-x-hidden">
      ...
      <div className="terminal-bg">
        ...
        {/* Navigation */}
        <nav className={`fixed z-20 w-full glass-card border-b border-white border-opacity-10 transition-all duration-300 ${scrollY > 50 ? "p-2 md:p-3" : "p-3 md:p-6"}`}>
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            <div className="flex items-center space-x-2 md:space-x-3">
              <div className={`bg-black bg-opacity-10 rounded border neon-border flex items-center justify-center neon-glow transition-all duration-300 ${scrollY > 50 ? "w-7 h-7 md:w-9 md:h-9" : "w-8 h-8 md:w-12 md:h-12"}`}>
                <Database className={`text-white transition-all duration-300 ${scrollY > 50 ? "w-3 h-3 md:w-5 md:h-5" : "w-4 h-4 md:w-7 md:h-7"}`} />
              </div>
              <div className="cursor-pointer">
                <h1 className={`font-bold mono-text cursor-pointer transition-all duration-300 ${scrollY > 50 ? "text-base md:text-lg" : "text-lg md:text-2xl"}`}>
                  CRUMBS
                </h1>
              </div>
            </div>
            ...
            <div className="hidden md:flex items-center space-x-6 lg:space-x-8">
              <a
                href="#features"
                className={`mono-text transition-colors text-sm lg:text-base ${activeSection === "features" ? "text-white border-b border-white" : "text-gray-400 hover:text-white"}`}
              >
                [features]
              </a>
              <a
                href="#algorithm"
                className={`mono-text transition-colors text-sm lg:text-base ${activeSection === "algorithm" ? "text-white border-b border-white" : "text-gray-400 hover:text-white"}`}
              >
                [algorithm]
              </a>
              <button
                className="mono-text text-gray-400 hover:text-white transition-colors cursor-pointer text-sm lg:text-base"
            ...
            {/* Mobile Menu Button */}
            <button
              className="md:hidden p-2 hover:bg-white hover:bg-opacity-10 rounded transition-all cursor-pointer"
        ...
        {mobileMenuOpen && (
          <div className="mobile-menu-overlay" onClick={() => setMobileMenuOpen(false)}>
            <div className="glass-card rounded-lg p-6 mx-4 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-lg font-bold mono-text">CRUMBS</h2>
                <button
              ...
              <div className="space-y-6">
                <a
                  href="#features"
                  className={`block mono-text transition-colors text-base ${activeSection === "features" ? "text-white" : "text-gray-400"}`}
                  onClick={() => setMobileMenuOpen(false)}
                >
                  [features]
                </a>
                <a
                  href="#algorithm"
                  className={`block mono-text transition-colors text-base ${activeSection === "algorithm" ? "text-white" : "text-gray-400"}`}
                  onClick={() => setMobileMenuOpen(false)}
                >
                  [algorithm]
                </a>
                <button
                  className="block w-full text-left mono-text text-gray-400 hover:text-white transition-colors text-base"
                  onClick={() => { setMobileMenuOpen(false); opencontact("/contact"); }}
                >
                  [contact]
                </button>
                <button
                  className="block w-full text-left mono-text text-gray-400 hover:text-white transition-colors text-base"
                  onClick={() => { setMobileMenuOpen(false); opencontact("/demo"); }}
                >
                  [demo]
                </button>
                <button
                  className="w-full bg-white text-black px-4 py-3 mono-text font-bold hover:bg-gray-200 transition-all neon-glow text-sm mt-4"
                  onClick={() => { setMobileMenuOpen(false); opencontact("/auth"); }}
                >
                  JOIN_NETWORK
                </button>
              </div>
        ...
        <section className="relative z-10 pt-24 sm:pt-32 md:pt-40 pb-12 md:pb-32">
          <div className="max-w-7xl mx-auto px-4 md:px-6 text-center">
            ...
            <div
              className="block sm:hidden mb-6 text-gray-400"
              style={{
                fontSize: "10px",
              }}
            >
            ...
            <div className="hidden sm:block ascii-art mb-8 text-center">
              ...
              >
                {/* Normal state */}
                <div className="crumbs-normal">{completeASCII}</div>
            ...
            <div className="glass-card rounded-lg p-4 md:p-8 mb-8 md:mb-12 max-w-4xl mx-auto neon-border">
              <div className="text-xs sm:text-sm md:text-lg lg:text-2xl mono-text text-gray-300 space-y-2 md:space-y-4">
                <p>
                  TRANSFORM IDLE SMARTPHONE STORAGE INTO POWERFUL P2P NETWORK
                </p>
                <p>
                  SAVE <span className="text-white font-bold">92.1%</span> ON
                  STORAGE COSTS
                </p>
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-8">
              {stats.map((stat, index) => (
                <div
                  key={index}
                  className="text-center glass-card rounded p-3 md:p-6 neon-border hover-lift data-stream"
                >
                  <div className="text-lg sm:text-2xl md:text-4xl font-bold mono-text mb-1 md:mb-2">
                    {stat.number}
                  </div>
                  <div className="text-gray-300 mono-text text-xs md:text-sm">
                    {stat.label}
                  </div>
                </div>
              ))}
        ...
        <section id="features" className="py-8 md:py-20">
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            <div className="text-center mb-6 md:mb-16">
              <h2
                className="text-2xl sm:text-3xl md:text-5xl font-bold mb-3 md:mb-6 mono-text glitch-text"
                data-text="FEATURES"
              >
                FEATURES
              </h2>
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            ...
            <div className="grid lg:grid-cols-2 gap-6 md:gap-12 items-start">
              <div className="glass-card rounded p-4 md:p-8 neon-border circuit-pattern">
                <h3 className="text-lg sm:text-xl md:text-2xl font-semibold mb-4 md:mb-6 mono-text">
                  PROCESS_FLOW:
                </h3>
              ...
              <div className="relative">
                <div className="glass-card rounded p-4 md:p-8 neon-border">
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:gap-3 md:gap-4">
                    {["NODE_A", "NODE_B", "NODE_C", "NODE_D"].map(
                      (node, index) => (
                        <div
                          key={index}
                          className="data-stream bg-black bg-opacity-10 rounded p-2 sm:p-3 md:p-4 text-center neon-border neon-glow"
                        >
                          <Smartphone className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 mx-auto mb-1 md:mb-2" />
                          <div className="text-xs md:text-sm font-semibold mono-text">
                            {node}
                          </div>
                          <div className="text-xs text-gray-300 mono-text">
                            [{[15, 25, 45, 50][index]}_SHARDS]
                          </div>
                        </div>
                      ),
                    )}
                  ...
                  <div className="mt-3 md:mt-6 text-center">
                    <div className="inline-block glass-card rounded px-2 sm:px-3 md:px-4 py-1 sm:py-2 neon-border">
                      ...
                      <span className="text-white mono-text text-xs md:text-sm">
                        OPTIMAL_DISTRIBUTION_ACHIEVED
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <section className="py-8 md:py-20">
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            <div className="text-center mb-6 md:mb-16">
              <h2
                className="text-2xl sm:text-3xl md:text-5xl font-bold mb-3 md:mb-6 mono-text glitch-text"
                data-text="TARGET_USERS"
              >
                TARGET_USERS
              </h2>
              <p className="text-sm sm:text-lg md:text-xl text-gray-300 mono-text">
                OPTIMIZED_FOR_FORWARD_THINKING_INDIVIDUALS
              </p>
            </div>
              ...
              {[
                {
                  ...
                  description:
                    "Access affordable, reliable storage for projects and media",
                },
                {
                  icon: (
                  ...
                  description:
                    "Complete control over data with end-to-end encryption",
                },
                {
                  icon: (
                    <Network className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8" />
                  ),
                  title: "EMERGING_MARKETS",
                  description:
                    "Democratizing cloud storage in expensive service regions",
                },
              ].map((audience, index) => (
                <div
                  key={index}
                  className="glass-card rounded p-4 md:p-8 neon-border text-center group hover-lift scanline"
                >
        ...
        <section className="py-8 md:py-20">
          <div className="max-w-4xl mx-auto px-4 md:px-6 text-center">
            <div className="glass-card rounded-lg p-4 sm:p-6 md:p-12 neon-border circuit-pattern">
              <h2
                className="text-xl sm:text-2xl md:text-4xl font-bold mb-4 md:mb-6 mono-text glitch-text"
                data-text="JOIN_REVOLUTION"
              >
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="text-sm sm:text-base md:text-lg font-bold mono-text">
                    PROJECT_CRUMBS
                  </div>
                  <div className="text-xs sm:text-sm text-gray-300 mono-text">
              ...
              <div className="text-center md:text-right">
                ...
                <p className="text-xs sm:text-sm text-gray-500 mono-text">
                  © {new Date().getFullYear()} PROJECT_CRUMBS. ALL_RIGHTS_RESERVED.
                </p>
              </div>
            </div>
          </div>
