...
const CrumbsP2PDemo = () => {
  ...
  const currentSteps =
    animationPhase === "upload" ? uploadSteps : downloadSteps;
    ...
    shards.forEach((shard, index) => {
      setTimeout(() => {
        ...
        animateShardMovement(newMovingShard, () => {
            ...
            prev.map((node) =>
              node.id === shard.node
                ? {
                    ...node,
                    shards: [
                      ...node.shards.filter((s) => s.id !== shard.id),
                      shard,
                    ],
                  }
                : node,
          ...
          setMovingShards((prev) =>
            prev.filter((ms) => ms.id !== newMovingShard.id),
          );
          ...
          setNodeStates((prev) =>
            prev.map((node) =>
              node.id === shard.node
                ? {
                    ...node,
                    shards: node.shards.filter((s) => s.id !== shard.id),
                  }
                : node,
            ),
          );
    ...
    <div className="min-h-screen bg-black text-white p-4 font-mono">  
        ...
        <div className="max-w-md mx-auto mb-6">
          <div className="bg-gray-900 border-2 border-dashed border-green-400 rounded-lg p-6 text-center">
            ...
            <p className="text-gray-400 text-sm mb-4">
              Choose a file to demonstrate P2P distribution
            </p>
            <button
      ...
      <div className="flex justify-center gap-4 mb-6">
        ...
        >
          ...
          {isAnimating && animationPhase === "upload"
            ? "UPLOADING..."
            : "START_UPLOAD"}
        </button>
        <button
        ...
        >
          ...
          {isAnimating && animationPhase === "download"
            ? "DOWNLOADING..."
            : "START_DOWNLOAD"}
        </button>
        <button
      ...
      <div className="bg-gray-900 border border-green-400 rounded-lg p-4 mb-6 max-w-4xl mx-auto">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span
            className={`font-bold ${animationPhase === "upload" ? "text-green-400" : "text-blue-400"}`}
          >
            {animationPhase === "upload"
              ? "UPLOAD_PROCESS:"
              : "DOWNLOAD_PROCESS:"}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div
              className={`text-sm ${isAnimating ? "text-yellow-400" : "text-green-400"}`}
            >
          ...
          <div>
            ...
            <div
              className={`text-sm ${animationPhase === "upload" ? "text-green-400" : "text-blue-400"}`}
            >
        ...
        <div className="mt-4">
          <div className="bg-gray-800 rounded-full h-3">
            <div
              ...
              style={{
                width: `${Math.min((currentStep / Math.max(currentSteps.length - 1, 1)) * 100, 100)}%`,
              }}
            />
          </div>
          <div className="text-xs text-gray-400 mt-1 text-center">
            {Math.min(currentStep, currentSteps.length - 1)}/
            {currentSteps.length - 1} STEPS COMPLETED
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div
          className="relative bg-gray-900 border border-green-400 rounded-lg overflow-hidden"
          style={{ height: "400px" }}
        >
          <svg
            width="100%"
            ...
            <div className="bg-gray-900 border border-green-400 rounded-lg p-4">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {shards.map((shard) => (
                  <div
                    key={shard.id}
                    className="flex justify-between items-center p-2 bg-gray-800 rounded text-xs"
                  >
